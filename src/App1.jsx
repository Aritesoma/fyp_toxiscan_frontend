...
import { Upload, ShieldCheck, AlertTriangle, Loader2, FileWarning, RefreshCcw, Activity } from 'lucide-react';
...
function App() {
  ...
  // Helper to determine UI colors based on the ML Risk Category
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans p-6 flex flex-col items-center">
      ...
      <main className="w-full max-w-lg bg-white shadow-xl rounded-3xl p-8 border border-slate-100">
        ...
        {!result && (
          <button
            ...
            className={`w-full mt-6 py-4 rounded-2xl font-bold text-white transition-all flex justify-center items-center gap-2 ${
              !file || loading ? 'bg-slate-300 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-100'
            }`}
          >
        ...
        {/* Results Box */}
        {result && (
          <div className={`mt-8 p-6 rounded-2xl border-2 animate-in fade-in zoom-in duration-300 ${getSeverityStyles(result.category).split(' icon')[0]}`}>
            
            <div className="flex justify-between items-start mb-4">
               <div>
                  <div className="flex items-center gap-2 mb-1">
                    {result.score > 4 ? <AlertTriangle size={20} className="text-orange-600" /> : <ShieldCheck size={20} className="text-green-600" />}
                    <h2 className="text-3xl font-black text-slate-800">{result.score}/10</h2>
                  </div>
                  <p className="text-xs font-bold uppercase text-slate-400 tracking-wider">Toxicity Score</p>
               </div>
               
               {/* Random Forest Category Badge */}
               <div className="text-right">
                  <div className={`px-4 py-1 rounded-full text-xs font-black uppercase tracking-tighter border-2 ${getSeverityStyles(result.category)}`}>
                      {result.category}
                  </div>
                  <p className="text-[10px] font-bold uppercase text-slate-400 mt-1">Classification</p>
               </div>
            </div>
            ...
            {result.detected_toxins.length > 0 && (
              <div className="mb-6">
                <p className="text-[10px] font-black uppercase text-slate-400 mb-2">Concerns Detected:</p>
                <div className="flex flex-wrap gap-2">
                  {result.detected_toxins.map((toxin, idx) => (
                    <span key={idx} className="bg-white/80 backdrop-blur-sm border border-slate-200 text-slate-700 px-3 py-1 rounded-lg text-xs font-bold capitalize shadow-sm">
                      {toxin}
                    </span>
                  ))}
                </div>
                      ...
                      <div className="mt-2 bg-white/50 p-3 rounded-lg border border-slate-100">
                        {result.ingredients_found.length > 0 ? (
                          result.ingredients_found.map((line, idx) => (
                            <p key={idx} className="text-[11px] text-slate-500 leading-relaxed py-1 border-b border-slate-100 last:border-0">
                              {line}
                            </p>
                          ))
                        ) : (
